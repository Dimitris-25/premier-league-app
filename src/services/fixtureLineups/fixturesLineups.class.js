...
class FixturesLineupsService {
  constructor(options = {}) {
    ...
    this.table = options.name || "fixturesLineUps"; // default to migrations name
    this.id = options.id || "lineup_id"; // default PK
  ...
  async find(params = {}) {
    ...
    console.log("[LINEUPS] find() query:", {
      where,
      limit,
      sortField,
      sortDir,
    ...
    try {
      ...
      console.log("[LINEUPS] find() returning rows:", rows.length);
      return rows;
    } catch (err) {
      console.error("[LINEUPS] find() error:", err.message);
      throw new Error("Failed to fetch lineups from DB: " + err.message);
    }
  }
  ...
  async toPlayerRowsAsync({ lineup_id, arr, isStarting }) {
    ...
    for (const it of arr) {
      ...
      rows.push({
        lineup_id,
        player_id, // NULL if not found (FK allows SET NULL)
